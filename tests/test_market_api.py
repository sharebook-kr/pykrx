...
class OhlcvByTickerTest(unittest.TestCase):
    def test_ohlcv_for_a_day(self):
        ...
        # 138930  BNK금융지주    5720    5720    5610    5610  2094260  11808815750
        temp = df.iloc[0:5, 0] == np.array(["AJ네트웍스", "AK홀딩스", "BGF", "BGF리테일", "BNK금융지주"])
        self.assertEqual(temp.sum(), 5)
        temp = df.iloc[0:5, 1] == np.array([4190, 25750, 5020, 156500, 5720])
        self.assertEqual(temp.sum(), 5)
    ...
    def test_ohlcv_for_a_day_in_kosdaq(self):
        ...
        #            종목명    시가    고가    저가    종가   거래량     거래대금
        # 티커
        # 060310         3S    2265    2290    2225    2255   275425    619653305
        # 054620  APS홀딩스    7210    7250    7030    7120   124636    883893780
        # 265520   AP시스템   25850   25850   25200   25400   196384   4994644750
        # 211270     AP위성   10250   10950   10050   10350  1664154  17351956900
        # 035760      CJENM  165200  166900  162600  163800   179018  29574003100
        temp = df.iloc[0:5, 0] == np.array(["3S", "APS홀딩스", "AP시스템", "AP위성", "CJENM"])
        self.assertEqual(temp.sum(), 5)
        temp = df.iloc[0:5, 1] == np.array([2265, 7210, 25850, 10250, 165200])
        self.assertEqual(temp.sum(), 5)
